...
class KSolver(pta: PTA) : Propagator() {
    ...
    override fun propagate() {
        val newRMs = rmQueue.reader()
        val newPAGEdges = edgeQueue.reader()
        val newThrows = throwSiteQueue.reader()
        val newCalls = virtualCallSiteQueue.reader()
    ...
    fun processStmts(newRMs: Iterator<MethodOrMethodContext>) {
        while (newRMs.hasNext()) {
            ...
            if (CoreConfig.v().ptaConfig.clinitMode == CoreConfig.ClinitMode.ONFLY) {
                ...
                while (it.hasNext()) {
                    val sm = it.next()
                    cgb.injectCallEdge(sm.declaringClass.type, pta.parameterize(sm, pta.emptyContext()), Kind.CLINIT)
                }
            }
    ...
    private fun recordCallStmts(m: MethodOrMethodContext, units: Collection<Unit>) {
        for (u in units) {
            val s = u as Stmt
    ...
    private fun addToPAG(mpag: MethodPAG, cxt: Context) {
        ...
        while (reader.hasNext()) {
            ...
            if (from is AllocNode && to is GlobalVarNode) {
                ...
                if (from is AllocNode) {
                    handleImplicitCallToFinalizerRegister(from)
                }
                pag.addEdge(from, to)
            }
        }
    ...
    private fun handleImplicitCallToFinalizerRegister(heap: AllocNode) {
    ...
    private fun handleStoreAndLoadOnBase(base: VarNode) {
        for (fr in base.allFieldRefs) {
            for (v in pag.storeInvLookup(fr)) {
                handleStoreEdge(base.p2Set.newSet, fr.field, v)
            }
            for (to in pag.loadLookup(fr)) {
                handleLoadEdge(base.p2Set.newSet, fr.field, to)
            }
        }
    }
